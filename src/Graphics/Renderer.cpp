...
void Renderer::createResources(int width, int height) {
    ...
    // Create an OpenCL image that shares memory with the OpenGL texture
    m_OutputImage = std::make_unique<cl::ImageGL>(*m_Context, CL_MEM_WRITE_ONLY, GL_TEXTURE_2D, 0, m_OutputTextureID);
...
void Renderer::render(IMetric* metric) {
    ...
    for (int y = 0; y < m_Height; ++y) {
        for (int x = 0; x < m_Width; ++x) {
            ...
            m_InitialRays[index].pos = Vec4(0.0, 0.0, -5.0, 0.0); // Camera at z = -5
            
            // Convert pixel coords to normalized device coords [-1, 1]
            float ndc_x = (2.0f * x / m_Width) - 1.0f;
            float ndc_y = 1.0f - (2.0f * y / m_Height);
            
            m_InitialRays[index].vel = glm::normalize(Vec4(ndc_x, ndc_y, 1.0, 0.0)); // Ray direction
        }
    }
    
    // Acquire the OpenGL texture for writing in OpenCL
    std::vector<cl::Memory> gl_objects = { *m_OutputImage };
    m_Queue->enqueueAcquireGLObjects(&gl_objects);
    ...
    // 5. Release the OpenGL texture
    m_Queue->enqueueReleaseGLObjects(&gl_objects);
    m_Queue->finish();
}
